...
public static class WidgetUtils
{
    ...
    public static unsafe void RenderTextClipped(ImDrawListPtr drawList, string text, Vector2 posMin, Vector2 posMax, Vector2 align,
        Vector2? textSize, ImRect? clipRect, bool checkSharp)
    {
        ...
        fixed (byte* start = bytes)
        {
            ...
            var size    = (ImVec2)(textSize ?? Vector2.Zero);
            var sizePtr = textSize.HasValue ? &size : null;
            var rect    = clipRect ?? default;
            var rectPtr = clipRect.HasValue ? &rect : null;
            ImGuiNativeAdditions.RenderTextClippedEx(drawList.NativePtr, posMin, posMax, start, start + numBytes, sizePtr, align, rectPtr);
        }
    }
    ...
    private static int GetEnd(ReadOnlySpan<char> text, bool checkSharp)
    {
        if (!checkSharp)
        {
